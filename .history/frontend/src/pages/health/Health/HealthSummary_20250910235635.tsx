...
function HealthSummary({ health, activities, nutrition, selectedDate, allHealthData = [], allActivities = [] }: Props) {
  ...
  const calculateBMI = () => {
    const currentHealth = getHealthForDate();
    if (!currentHealth || currentHealth.weight === 0 || currentHealth.height === 0) return 0;
    ...
    <div style={{
      ...
      <div style={{
        ...
        {(() => {
          ...
          return (
            <p style={{
              color: hasDataForDate ? "#10b981" : "#f59e0b",
              margin: "0.5rem 0 0 0",
              fontSize: "0.85rem",
              fontWeight: "500"
            }}>
              {hasDataForDate ? "📅 ข้อมูลของวันที่นี้" : "📅 แสดงข้อมูลล่าสุด"}
            </p>
          );
        })()}
      </div>
