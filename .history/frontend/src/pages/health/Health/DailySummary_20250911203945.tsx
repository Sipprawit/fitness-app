...
function DailySummary({ health, activities, selectedDate }: Props) {
  if (!health) {
    return (
  ...
  // กรองกิจกรรมตามวันที่เลือก (ถ้ามี) หรือแสดงของวันนี้
  ...
  // Debug logging
  console.log("=== DailySummary Debug ===");
  console.log("selectedDate:", selectedDate);
  console.log("targetDate:", targetDate);
  console.log("activities:", activities);
  console.log("filteredActivities:", filteredActivities);
  console.log("=== End DailySummary Debug ===");

  const totalCaloriesBurned = filteredActivities.reduce((total, activity) => {
    ...
    <div style={containerStyle}>
      <div style={headerStyle}>
        ...
        <p style={subtitleStyle}>
          {selectedDate 
            ? `ข้อมูลสุขภาพและกิจกรรมของคุณ (${selectedDate})`
            : `ข้อมูลสุขภาพและกิจกรรมของคุณ (วันนี้ - ${targetDate})`
          }
        </p>
      </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={dataGridStyle}>
            <div style={dataItemStyle}>
              ...
              <span style={dataValueStyle}>{filteredActivities.length} กิจกรรม</span>
            </div>
            <div style={dataItemStyle}>
              <span style={dataLabelStyle}>แคลอรี่เผาผลาญ</span>
              <span style={dataValueStyle}>{totalCaloriesBurned.toFixed(2)} แคลอรี่</span>
            </div>
            <div style={dataItemStyle}>
              <span style={dataLabelStyle}>เวลารวม</span>
              <span style={dataValueStyle}>
                {filteredActivities.reduce((total, activity) => total + (activity.duration || 0), 0)} นาที
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Date */}
      <div style={dateStyle}>
        ข้อมูลสุขภาพล่าสุด: {health.date}
        {filteredActivities.length === 0 && (
          <div style={{ marginTop: "0.5rem", color: "#ef4444", fontSize: "0.8rem" }}>
            ไม่มีกิจกรรมในวันที่ {targetDate}
          </div>
        )}
      </div>
    </div>
