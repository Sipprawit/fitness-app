...
function HealthSummary({ health, activities, nutrition, selectedDate, allHealthData, allActivities }: Props) {
  ...
  const getHealthForDate = () => {
    console.log("=== getHealthForDate Debug ===");
    console.log("allHealthData:", allHealthData);
    console.log("selectedDate:", selectedDate);
    console.log("health (fallback):", health);
    
    if (!allHealthData || allHealthData.length === 0) {
      console.log("No allHealthData, returning health:", health);
      return health;
    }
    
    // หาข้อมูลของวันที่เลือก
    const healthForDate = allHealthData.find(h => h.date === selectedDate);
    console.log("healthForDate:", healthForDate);
    
    if (healthForDate) {
      console.log("Found health for selected date:", healthForDate);
      return healthForDate;
    }
    ...
    const latestHealth = sortedHealth[0] || health;
    console.log("Latest health before selected date:", latestHealth);
    console.log("=== End getHealthForDate Debug ===");
  ...
  const calculateBMI = () => {
    const currentHealth = getHealthForDate();
    console.log("=== calculateBMI Debug ===");
    console.log("currentHealth:", currentHealth);
    
    if (!currentHealth || !currentHealth.weight || currentHealth.weight === 0 || !currentHealth.height || currentHealth.height === 0) {
      console.log("Invalid health data for BMI calculation");
      return 0;
    }
    
    const heightInMeters = currentHealth.height / 100;
    const bmi = (currentHealth.weight / (heightInMeters * heightInMeters)).toFixed(1);
    console.log(`BMI calculation: ${currentHealth.weight} / (${heightInMeters} * ${heightInMeters}) = ${bmi}`);
    console.log("=== End calculateBMI Debug ===");
