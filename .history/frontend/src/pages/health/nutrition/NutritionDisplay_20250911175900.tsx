...
function NutritionDisplay({ 
  ...
  const handleNutritionSubmit = async (data: { goal: string; total_calories_per_day: number; note?: string; }) => {
    await onNutritionSubmit({ ...data, date: selectedDate });
    // ไม่ต้อง setIsEditingNutrition(false) ที่นี่ เพราะจะทำใน NutritionForm
  };
  ...
  const handleCancelEditNutrition = () => {
  ...
  return (
    <div style={{ 
      ...
      {/* Nutrition Form with Health Display - แสดงเมื่อไม่มีข้อมูลเป้าหมาย หรือกำลังแก้ไข */}
      {(!latestNutrition || isEditingNutrition) && (
        <div style={{ 
          display: "flex", 
          gap: "2rem", 
          flexWrap: "wrap", 
          ...
          <NutritionForm
            ...
            currentNote={latestNutrition?.note}
            isEditing={isEditingNutrition}
            onCancelEdit={handleCancelEditNutrition}
          />
          <HealthDisplay health={health} />
        </div>
      )}

      {/* Health Display แสดงเมื่อมีข้อมูลเป้าหมายแล้วและไม่ได้แก้ไข */}
      {latestNutrition && !isEditingNutrition && (
        <div style={{ 
          display: "flex", 
          gap: "2rem", 
      ...
      {/* Meal Plan - แสดงแผนมื้ออาหารของวันที่เลือก หรือล่าสุดถ้าไม่มีข้อมูลของวันที่เลือก */}
      {currentNutrition ? (
        <div>
          <h3 style={{
            color: "#c50000",
            ...
            🍽️ แผนมื้ออาหารวันนี้ ({selectedDate})
          </h3>
          <MealPlan 
        ...
        <div>
          <h3 style={{
            ...
            🍽️ แผนมื้ออาหารล่าสุด ({latestNutrition.date})
          </h3>
          <div style={{
            backgroundColor: "rgba(255, 255, 255, 0.9)",
            padding: "1rem",
            ...
            <p style={{
              ...
              ไม่มีข้อมูลแผนมื้ออาหารสำหรับวันที่ {selectedDate}
            </p>
            <p style={{
              color: "#c50000",
              margin: 0,
              ...
              แสดงแผนมื้ออาหารล่าสุดจากวันที่ {latestNutrition.date}
            </p>
          </div>
          <MealPlan 
            totalCaloriesGoal={latestNutrition.total_calories_per_day}
            proteinG={latestNutrition.protein_g}
            fatG={latestNutrition.fat_g}
            carbG={latestNutrition.carb_g}
          />
        </div>
      ) : (
        <div style={{
          ...
          <p style={{
            ...
            ยังไม่มีข้อมูลแผนมื้ออาหาร กรุณากรอกเป้าหมายโภชนาการก่อน
          </p>
        </div>
      )}
    </div>
