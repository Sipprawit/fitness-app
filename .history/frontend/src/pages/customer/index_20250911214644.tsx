...
function ProfileCustomer() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setIsEditing(false);
        showNotification("บันทึกข้อมูลสำเร็จ!", "success", 2000);
      } else {
        showNotification("เกิดข้อผิดพลาดในการบันทึกข้อมูล", "error", 3000);
      }
    } catch (error) {
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      showNotification("กรุณาเลือกไฟล์รูปภาพเท่านั้น", "error", 3000);
      return;
    }

    // Validate file size (max 5MB)
    if (file.size > 5 * 1024 * 1024) {
      showNotification("ขนาดไฟล์ต้องไม่เกิน 5MB", "error", 3000);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification("อัปโหลดรูปโปรไฟล์สำเร็จ!", "success", 2000);
      } else {
        showNotification("เกิดข้อผิดพลาดในการอัปโหลดรูป", "error", 3000);
      }
    } catch (error) {
        ...
        <div style={infoSectionStyle}>
          {isEditing ? (
            ...
            <div style={infoDisplayStyle}>
              ...
              <div style={infoItemStyle}>
                <span style={infoLabelStyle}>🏃‍♂️ สมาชิกฟิตเนสตั้งแต่:</span>
