import { Button, Card, Form, Input, Flex, Row, Col, InputNumber, DatePicker, Select } from "antd";
  ...
  const onGetGender = async () => {
    ...
    if (res.status == 200) setGender(res.data);
      ...
      showNotification({
        ...
        message: 'ไม่พบข้อมูลเพศ กรุณาลองใหม่อีกครั้ง',
        duration: 4000
      });
    ...
    try {
      ...
      if (res?.status == 201) {
        showNotification({
          ...
          message: res.data.message || 'ยินดีต้อนรับสู่ระบบ Fitness App',
          duration: 4000
        });
