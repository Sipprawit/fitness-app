...
import { GetAllClasses, GetTrainers, CreateReview, GetUserBookedClasses, GetUserBookedTrainers } from '../../services/https';
    ...
    try {
      ...
      const formattedData: TrainingItem[] = await Promise.all(raw.map(async (booking: any) => {
        ...
        if (filter === 'คลาส') {
          ...
          // สำหรับเทรนเนอร์: ข้อมูลอาจอยู่ใน trainer หรือ trainer_id
          item = booking.trainer || booking.Trainer;
          if (!item && booking.trainer_id) {
            ...
            try {
              ...
              if (trainerResponse.status === 200 && Array.isArray(trainerResponse.data)) {
                const trainer = trainerResponse.data.find((t: any) => t.id === booking.trainer_id || t.ID === booking.trainer_id);
                if (trainer) {
                  item = trainer;
                  console.log(`Found trainer data for ID ${booking.trainer_id}:`, trainer);
                } else {
                  // ถ้าไม่เจอเทรนเนอร์ ให้สร้างข้อมูลเบื้องต้น
                  item = { id: booking.trainer_id, first_name: 'เทรนเนอร์', last_name: `ID: ${booking.trainer_id}` };
                }
              } else {
              ...
              console.error(`Failed to fetch trainer data for ID ${booking.trainer_id}:`, error);
