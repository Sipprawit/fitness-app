...
import { GetTrainers, CreateReview, GetUserBookedClasses, GetUserBookedTrainers, GetClassReviews, GetReviewsByItem } from '../../services/https';
      ...
      const formattedData: TrainingItem[] = await Promise.all(raw.map(async (booking: any) => {
        ...
        if (rawReviews.length === 0) {
          try {
            ...
            let reviewResponse;
            if (filter === 'คลาส') {
              reviewResponse = await GetClassReviews(item.ID || item.id);
            } else {
              // สำหรับเทรนเนอร์ ให้ดึงข้อมูลรีวิวจาก GetTrainers API
              const trainerResponse = await GetTrainers();
              if (trainerResponse.status === 200 && Array.isArray(trainerResponse.data)) {
                const trainer = trainerResponse.data.find((t: any) => (t.id === item.ID || t.ID === item.ID) || (t.id === item.id || t.ID === item.id));
                if (trainer && trainer.reviews) {
                  rawReviews = Array.isArray(trainer.reviews) ? trainer.reviews : [];
                  console.log(`Found reviews in trainer data:`, rawReviews);
                }
              }
            }
